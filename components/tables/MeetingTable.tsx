...
export const MeetingTable: React.FC<MeetingTableProps> = ({ data, onNameClick }) => {
  // Override ID width for Meetings as UUIDs are longer and text is larger
  const MEET_ID_WIDTH = "w-[240px] min-w-[240px]";
    ...
    <div className="w-full h-full overflow-auto pb-20 font-sans">
      <table className="min-w-max border-separate border-spacing-0">
        <thead className="bg-white sticky top-0 z-40 shadow-sm shadow-gray-100/50">
          <tr>
            <th className={`sticky left-0 z-50 bg-white px-6 py-5 text-left text-sm font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 ${MEET_ID_WIDTH} shadow-[4px_0_12px_-4px_rgba(0,0,0,0.02)]`}>
              Meeting ID
            </th>
        ...
        <tbody className="bg-white">
          {data.map((row) => (
            <tr key={row.id} className="group/row hover:bg-gray-50">
              <td className={`sticky left-0 z-30 bg-white group-hover/row:bg-gray-50 px-6 py-5 whitespace-nowrap text-sm font-mono border-b border-gray-50 ${MEET_ID_WIDTH} shadow-[4px_0_12px_-4px_rgba(0,0,0,0.02)]`}>
                 <button 
              ...
              <Cell className={`${COL.TEXT_MD} font-medium text-base text-gray-600`}>{row.vc_id}</Cell>
              <Cell className={`${COL.DATE} text-base text-gray-500`}>{new Date(row.start_time).toLocaleString('en-US', { weekday: 'short', hour: 'numeric', minute: 'numeric', month: 'short', day: 'numeric' })}</Cell>
              <Cell className={`${COL.DATE} text-base text-gray-400`}>{row.end_time ? new Date(row.end_time).toLocaleTimeString() : '-'}</Cell>
              <Cell className={COL.STATUS}>
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${
                  ...
                  'bg-rose-50 text-rose-500 border border-rose-100'
                   ...
                   {row.status === 'in_progress' && <span className="w-2 h-2 bg-indigo-500 rounded-full mr-2"></span>}
