...
export const ExpandableContent = ({ content, widthClass = COL.TEXT_LG }: { content: string, widthClass?: string }) => {
  return (
    <div className={`group relative ${widthClass}`}>
      <div className="truncate text-sm text-gray-500 leading-relaxed cursor-default w-full font-medium">
        {content}
      </div>
      <div className="hidden group-hover:block absolute left-0 -top-2 z-50 bg-gray-900 text-white p-4 rounded-lg shadow-xl shadow-gray-200 animate-in fade-in zoom-in-95 duration-200 w-full min-w-[320px] max-w-[400px]">
...
export const HeaderCell = ({ children, className = '', width = '' }: { children?: React.ReactNode, className?: string, width?: string }) => (
  <th className={`px-4 py-3 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-white border-b border-gray-100 align-middle ${className} ${width}`}>
    <div className="flex items-center gap-1.5 cursor-pointer group whitespace-nowrap hover:text-gray-800 transition-colors">
      {children}
    </div>
  </th>
);
...
export const Cell = ({ children, className = '', ...props }: React.TdHTMLAttributes<HTMLTableCellElement>) => (
  <td className={`px-4 py-3 text-sm text-gray-600 border-b border-gray-50 bg-white align-middle transition-colors group-hover/row:bg-gray-50 ${className}`} {...props}>
    {children}
  </td>
);
